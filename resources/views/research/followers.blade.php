        ...
        <div class="col-md-9">
            ...
            <div class="p-2 bg-white mb-3">
                <ul class="nav nav-pills">
                    ...
                    <li class="nav-item mr-sm-2">
                        <a class="nav-link active" href="{{ route('researcher.followers', $account->id) }}">Followers <span class="badge badge-primary">{{ $account->followers()->count() }}</span></a>
                    </li>
                </ul>
            </div>
            <!-- End Nav Bars -->

            <h3 class="mb-5">Followers <span class="badge badge-primary">{{ $account->followers()->count() }}</span></h3>
            ...
            <div class="container">
                <div class="row">
                    @forelse ( $account->followers as $follower)
                        <div class="col-md-6 mb-3">
                            <div class="card mr-3 h-100">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-4 text-center">
                                            <a href="{{ route( 'researcher.index', $follower->user->id ) }}">
                                                <img src="{{ asset($follower->user->avatar) }}" alt="user" class="card-img rounded-circle" style="width: 100%">
                                            </a>
                                        </div>
                                        <div class="col-md-8">
                                            <h5 class="card-title mb-0">
                                                ...
                                                {!! $follower->user->isFollowing( auth()->user()->id ) ? '<small class="badge badge-secondary">Follows you</small>' : '' !!}
                                            </h5> 
                                            
                                            <small>{{ $follower->user->is_researcher ? 'Researcher' : '' }}&nbsp;</small>
                                            @if ( $follower->user->id != auth()->user()->id )
                                                @if ( auth()->user()->isFollowing( $follower->user->id ) )
                                                    <!-- Unfollow Button -->
                                                    <form action="{{ route('researcher.unfollow', $follower->user->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-outline-primary">Unfollow</button>
                                                ...
                                                @else
                                                    <!-- Follow Button -->
                                                    <form action="{{ route('researcher.follow', $follower->user->id ) }}" method="POST">
                                                        @csrf
                                            ...
                                            <address>
                                                ...
                                                <br>
                                                <br>
                                                <abbr title="Email"><i class="fas fa-envelope"></i></abbr> <a href="mailto:{{ $follower->user->email }}" target="_top">{{ $follower->user->email }}</a>
                                                <br>
